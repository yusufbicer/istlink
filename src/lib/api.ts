  ...
  getCustomers: async (supplierOnly: boolean = false): Promise<Customer[]> => {
    try {
      ...
      let customersQuery = supabase.from('customers').select(`
        ...
        user_id,
        company_name,
        profiles:profiles(id, name, email, role)
      `);
  ...
  getOrders: async (filterByUser: boolean = true): Promise<Order[]> => {
    try {
      ...
      let query = supabase.from('orders').select(`
        ...
        order_date,
        total_amount,
        customers:customers(id, company_name, profiles:profiles(name)),
        suppliers:suppliers(id, company_name, profiles:profiles(name))
      `);
